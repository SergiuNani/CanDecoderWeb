...
const Decode_CAN_LOG_Window = () => {
  ...
  const [hideTableForceParentToggle, sethideTableForceParentToggle] =
    useState(false);
  const [shortcutToDecodeMessages, setShortcutToDecodeMessages] =
    useState(false);
  ...
  const { freeTextVsCanLog, toggleSearchWindow_app } = useContext(
    DecodeCANlog_topbarOptionsContext
  );
...
const DecodedTableOptions = ({ fileInnerText }) => {
  ...
  var { Clearance } = useContext(ClearanceContext);
  const { toggleFilterWindow_app } = useContext(
    DecodeCANlog_topbarOptionsContext
  );
  ...
  const showCANReal_Memo = useMemo(() => {
    return (
      showCANReal && (
        <CANRealComponent
          showCANReal={showCANReal}
          setShowCANReal={setShowCANReal}
        />
      )
    );
  }, [showCANReal]);
  const showHomeWindow_Memo = useMemo(() => {
    return (
      showHomeWindow && (
        <HomeComponent
          showHomeWindow={showHomeWindow}
          setShowHomeWindow={setShowHomeWindow}
        />
      )
    );
  }, [showHomeWindow]);
...
const DrawerComponent_DecodeOptions = ({
  ...
  const { shortcutToDecodeMessages, shortcutToDecodeMessages_whoCalled } =
  ...
  function handleDECODE() {
    ...
    setTimeout(
      () => {
        ...
        filteredMessages_g = filterMessagesByAxesAndCobID(
          filteredMessages_g,
          messageTypeSorting
        );
...
const AvailableAxes_Component = () => {
  ...
  function handleChecboxClicked(e) {
    ...
    if (arrayIndex != -1) {
      ...
      const allPropsAreTheSameState = Object.keys(
        CanLogStatistics[arrayIndex]
      ).every((prop) => {
        return (
          prop === "Axis" ||
          prop === propToChange ||
...
const MappingWindowforDrawer = ({
  ...
  return (
    <Dialog
      open={showMappingWindow}
      onClose={() => setShowMappingWindow(false)}
    >
      <div
        style={{
          border: `1px solid ${colors.primary[400]}`,
...
const MessageWindowForDrawerComponent = ({
  showRawMsgsWindow,
  setShowRawMsgsWindow,
}) => {
  const theme = useTheme();
      ...
      >
        ...
        <textarea
          ...
          defaultValue={filteredMessages_auxGlobal
            .map(
              (iteration) =>
                `[${iteration.msgNr}] --  ${iteration.OriginalMessage}`
            )
            .join("\n")}
          style={{
            background: `${colors.primary[300]}`,
...
const AdvancedSearchComponent = () => {
  ...
  const { isAdvancedSearchOpen, setIsAdvancedSearchOpen, SearchVsGotoLineRef } =
    useContext(Decode_CAN_LOG_WindowContext);
  ...
  function handleUserInput(e) {
    if (gotoLineStatus) {
      ...
      var potentialGroup =
        Table_NRs[index].closest("table").parentElement.parentElement;
      if (
      ...
      var FilterResult = array.filter((iteration) => {
        ...
        return searchProperties.some(
          ({ key, enabled }) =>
            enabled &&
            axisID &&
            iteration[key]?.toString().toLowerCase().includes(searchValue)
        );
      });
...
const ShowTimeWindowComponent = ({ showTime, setShowTime }) => {
  ...
  function ExtractTimeFromFrame(strInput, previousElement) {
    ...
    if (SingleColumn_flag != -1 && DoubleColumn_flag == -1) {
      ...
      //13:56:21:0411
      strInput[ThreeColumn_flag] = strInput[ThreeColumn_flag].replace(
        /^(.*?:.*?:.*?):/,
        "$1."
      );
    ...
    >
      ...
      >
        ...
        >
          ...
          <li>
            Only the first 1000 messages will be shown. To view the next batch,
            modify the "Cut the Log between" option.
          </li>
        </ul>
        <section>
          {arrCopy.length > 0 ? (
