  ...
  useEffect(() => {
    const handleKeyDown = (event) => {
      if (event.ctrlKey && event.key === "`") {
        ...
        var allValueInputs = document.querySelectorAll(
          ".RegisterNameInput_class"
        );
        ...
        event.preventDefault();
        var allValueInputs = document.querySelectorAll(
          ".RegisterComponent_class"
        );
        if (registerComponentRef.current == windowsNumber)
          registerComponentRef.current = 0;
        registerComponentRef.current += 1;
        allValueInputs[registerComponentRef.current - 1].focus();
      } else if (event.key === "ArrowRight") {
        var allValueInputs = document.querySelectorAll(
          ".RegisterValueInput_class"
        );
...
export const RegisterSelectionComponent = ({
  ...
  function tellParentRegisterChanged(register) {
    ...
    setRegisterResolution(
      parseInt(getMaxNumberFromStringRange(register.BitInfo[0].bit) + 1)
    );
    setValueRegister("");
  }
      ...
      >
        <AutocompleteInput_RegisterList
          ...
          stopResetValueofInputFromParent={
            stopResetValueofInputFromParent.current
          }
          placeholder="Select"
    ...
    >
      <div>
        <Typography variant="h5">
          The <span className="primaryColor">Register Bit Representation</span>{" "}
          window was build to easily visualize any registers from the CANopen
          Standard (CiA 402 v4.0) or TML Registers from Technosoft.{" "}
        </Typography>
        <br />
        <p>
          A <span className="primaryColor">REGISTER</span> is a key part of a
          computer's central processing unit (CPU), which plays a fundamental
          role in temporarily storing and managing data during the execution of
          instructions. These registers are usually divided into bits, where
          each bit signifies a specific action or represents a unit of
          information.
        </p>
        <br />
        <p>
          This interface saves users from manually searching through the CiA
          standard or the Technosoft Help menu. It eliminates the need to access
          specific register details, convert register values into binary, and
          link bit descriptions to their corresponding values.
        </p>
        <br />
        ...
        <ol className="ol">
          <li>
            <span className="primaryColor">Autocomplete Search Bar:</span>{" "}
            Quickly find any register by clicking on the search bar. To trigger
            updates, it's necessary not only to type the register name but also
            to either click on it or navigate through the generated list using
            the arrow keys followed by the ENTER key. Once a register is
            selected, the tool generates a visual interface.
          </li>
          <li>
            {" "}
            <span className="primaryColor">HEX/DEC Toggle Button: </span>
            The next part is a button that switches between HEX (hexadecimal)
            and DEC (decimal). Its purpose is to let the user choose the format
            for entering the register value.
          </li>
          <li>
            <span className="primaryColor">Value Input Bar:</span> This input
            bar allows users to specify the register data, either in decimal or
            hexadecimal, depending on the previous button selection. The visual
            representation of the register updates in real-time as the input
            value changes.
          </li>

          <li>
            <span className="primaryColor">CANopen/Technosoft Toggle:</span>{" "}
            This button provides the option to switch between "CANopen" and
            "Technosoft" registers. It helps users differentiate between CiA
            registers and Technosoft TML registers.
          </li>
          <li>
            <span className="primaryColor">Clone Button (+):</span> The "+"
            button can replicate the current tool, allowing for up to three
            separate instances. Each clone retains the same functionality and
            operates independently. This feature is useful when users need to
            query multiple registers or compare them. Clones of the Register
            tool can be deleted at any time by clicking the close button.
          </li>
          <li>
            <span className="primaryColor">Generated Register Display:</span>{" "}
            The sixth component is the generated register itself. It consists of
            multiple lines, with each line indicating the bit number, its
            description, and its current value. You can click on the box
            containing the value of a specific bit, this action will negate the
            previous value, simultaneously updating the "Value Input Bar" (3) of
            the register. This feature proves helpful when you need to know what
            value the register will have if one or more bits have changed.
          </li>
        </ol>
      </div>
