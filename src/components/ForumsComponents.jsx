...
export function AutocompleteInput_RegisterList({
  ...
  const filterOptions = (value, whoCalled) => {
    ...
    if (whoCalled == "InputChange") {
      if (
        CANRegResult.length == 1 &&
        CANRegResult[0].Index.toLowerCase() == value.toLowerCase()
      ) {
  ...
  const handleBlur = () => {
    ...
    setTimeout(() => {
      if (
        ...
        !divParentRef.current.contains(document.activeElement)
      ) {
        setIsFocused(false);
        setFilteredOptions([]); // Hide options when blurred
      }
    ...
    if (inputRef.current.contains(event.target)) {
      setInputValue((prevInputValue) => {
        const currentIndex = options.findIndex(
          (iterate) => iterate == prevInputValue
        );
